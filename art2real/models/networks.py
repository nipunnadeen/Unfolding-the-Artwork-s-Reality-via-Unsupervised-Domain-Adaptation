...
def define_adpt_net(input_nc, netC, init_type='normal', init_gain=0.02, gpu_ids=[]):
    n_network = None
    if netC == "lenet":
        n_network = LeNet(input_nc)
    elif netC == "des_feature":
        n_network = FeatureDiscriminator()

    return init_net(n_network, init_type, init_gain, gpu_ids)
...
class LeNet(nn.Module): #for 32
    def __init__(self, input_image_nc):
        super(LeNet, self).__init__()

        self.net = [
            nn.Conv2d(input_image_nc, 20, kernel_size=5, stride=1, padding=0),
            ...
            nn.Linear(50*5*5, 500, bias=True),
            nn.ReLU(inplace=True),
            nn.Dropout(0.8),
...
class FeatureDiscriminator(nn.Module):
    ...
    def forward(self, dis_score):
        return self.discriminator(dis_score)
